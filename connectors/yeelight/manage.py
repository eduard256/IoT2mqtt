#!/usr/bin/env python3
"""
Yeelight Connector - Management CLI
Manage existing Yeelight instances (edit, sync, delete)
"""
...
class YeelightManager:
    """Manage Yeelight Connector instances"""
    ...
    def show_instances_list(self):
        ...
        console.print(Panel.fit(
            "[bold cyan]Yeelight Connector - Instance Manager[/bold cyan]",
            border_style="cyan",
            box=box.DOUBLE
        ))
    ...
    def sync_devices(self):
        """Sync devices from network discovery"""
        console.print("\n[cyan]Discovering Yeelight devices on network...[/cyan]")
        ...
        try:
            ...
            if not bulbs:
                console.print("[yellow]No new devices found[/yellow]")
            else:
                console.print(f"\n[green]Found {len(bulbs)} device(s)[/green]")
                ...
                if new_devices:
                    console.print(f"\n[cyan]Found {len(new_devices)} new device(s):[/cyan]")
                    for device in new_devices:
                        console.print(f"  • {device['name']} ({device['ip']})")
                    
                    if Confirm.ask("\nAdd these devices?", default=True):
                        for device in new_devices:
                            config['devices'].append({
                                'device_id': f"yeelight_{device['id']}",
                                'global_id': f"{config['instance_id']}_yeelight_{device['id']}",
                        ...
                        self.save_instance_config(self.selected_instance, config)
                        console.print(f"[green]Added {len(new_devices)} device(s)[/green]")
                        
                        if self.get_instance_status(self.selected_instance) == "[green]Running[/green]":
                            if Confirm.ask("Restart container to apply changes?", default=True):
                                self.restart_container()
                else:
                    console.print("[yellow]All discovered devices are already configured[/yellow]")
                    
        except ImportError:
            console.print("[red]yeelight library not installed[/red]")
        except Exception as e:
